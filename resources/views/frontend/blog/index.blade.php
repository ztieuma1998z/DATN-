                        ...
                        @foreach($blogs as $blog)
                            ...
                            <div class="col-lg-9 col-md-9">
                                <div class="feature-img">
                                    <img class="img-fluid" src="{{ isset($blog->thumbnail) ? asset('storage/'.$blog->thumbnail) : asset('admins/img/default-image.jpg') }}" alt="" />
                                </div>
                                <a class="posts-title" href="{{ route('get.blog.detail', [$blog->url_key, $blog->id]) }}">
                                    <h3>{{ $blog->title }}</h3>
                                </a>
                                <p class="excert desc-course">
                                    {{ $blog->description }}
                                </p>
                                <a href="{{ route('get.blog.detail', [$blog->url_key, $blog->id]) }}" class="primary-btn">Xem thêm</a>
                            </div>
                        </div>
                        @endforeach
                    @endif
