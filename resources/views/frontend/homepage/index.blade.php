        ...
        <div class="row fullscreen d-flex align-items-center justify-content-between">
            <div class="banner-content col-lg-9 col-md-12">
                <h1 class="text-uppercase">
                    {{ isset($setting->banner_title) ? $setting->banner_title : '' }}
                </h1>
                <p class="pt-10 pb-10">
                    {{ isset($setting->banner_description) ? $setting->banner_description : '' }}
                </p>
                <a href="#section1" class="primary-btn text-uppercase">Bắt đầu</a>
            </div>
        </div>
    </div>
@stop
...
@section('content')
    ...
    <section style="background:url('{{ isset($setting->banner_home) ? asset('storage/'.$setting->banner_home) : asset('admins/img/default-image.jpg') }}') center;background-size: cover;" class="search-course-area relative" id="section1">
        <div class="overlay overlay-bg"></div>
        <div class="container">
                ...
                @if(isset($newBlog))
                    @foreach($newBlog as $item)
                        <div class="col-lg-3 col-md-6 single-blog">
                            <div class="thumb">
                                <img class="img-fluid" src="{{ isset($item->thumbnail) ? asset('storage/'.$item->thumbnail) : asset('admins/img/default-image.jpg') }}" alt="" />
                            </div>
                            <p class="meta">{{ $item->created_at->format('d-m-Y') }} | <a href="#">{{ isset($item->admin->full_name) ? $item->admin->full_name : '[N/A]'  }}</a></p>
                            <a href="{{ route('get.blog.detail', [$item->url_key, $item->id]) }}">
                                <h5>{{ $item->title }}</h5>
                            </a>
                            <p>
                                {{ $item->description }}
                            </p>
                            <a href="{{ route('get.blog.detail', [$item->url_key, $item->id]) }}" class="details-btn d-flex justify-content-center align-items-center">
